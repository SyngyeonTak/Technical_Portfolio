...
def get_train_loader(model_name, train_data, n_node = None, args = None, shuffle = False):
    if model_name == 'NARM':
        from model.NARM.dataset_rep import RecSysDataset
        from model.NARM.utils_rep import collate_fn
        ...
        from model.SR_GNN.pytorch_code.utils_rep import Data
        return Data(train_data, shuffle=shuffle)

    
    elif model_name == 'SHARE':
        from model.SHARE.utils_rep import Data
        train_loader = Data(train_data, args.window)
        return train_loader

    elif model_name == 'GNN_AM':
        from model.GNN_AM.utils_rep import Data
        return Data(train_data, shuffle=shuffle)
...
def get_model(model_name, args, n_node, original_dataset = None):
    if model_name == 'NARM':
        from model.NARM.narm_rep import NARM
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        model = NARM(n_node, args).to(device)
        return model

    elif model_name == 'SR_GNN':
        from model.SR_GNN.pytorch_code.model_rep import SessionGraph
        model = trans_to_cuda(SessionGraph(args, n_node))
        return model

    elif model_name == 'GNN_AM':
        from model.GNN_AM.model_rep import SessionGraph
        model = trans_to_cuda(SessionGraph(args, n_node))
        return model
    
    elif model_name == 'SHARE':
        from model.SHARE.model_rep import SessionGraph
        model = trans_to_cuda(SessionGraph(args, n_node))
        return model
...
def get_train_test_results(model_name, model, train_loader, test_loader, group_dict, seed):
    if model_name == 'NARM':
        from model.NARM.narm_rep import train_test_ht_sl
    elif model_name == 'SR_GNN':
        from model.SR_GNN.pytorch_code.model_rep import train_test_ht_sl
    
    elif model_name == 'SHARE':    
        from model.SHARE.model_rep import train_test_ht_sl    
    elif model_name == 'GNN_AM':    
        from model.GNN_AM.model_rep import train_test_ht_sl
