...
class SessionGraph(Module):
    ...
    def compute_scores(self, hidden, mask, flags=None, session_ids=None, return_alpha=False):
        ...
        a = self.linear_transform(torch.cat((ais.squeeze(-1), ht0), 1))
        ...
        if return_alpha:
            sid_attn_pairs = []
            # [batch, seq_len] 형태로 정리 (헤드 차원은 평균 처리 or squeeze)
            if weights.dim() == 3:
                weights_np = weights.mean(-1).detach().cpu().numpy()  # 여러 head → 평균
            else:
                weights_np = weights.detach().cpu().numpy()

            mask_np = mask.cpu().numpy()

            for sid, attn_row, m_row, flag in zip(session_ids, weights_np, mask_np, flags):
                if flag:
                    true_len = int(m_row.sum())
                    #sid_attn_pairs.append((sid, attn_row[:true_len]))

                    attn_tensor = torch.tensor(attn_row[:true_len])
                    attn_softmax = F.softmax(attn_tensor, dim=-1)
                    sid_attn_pairs.append((sid, attn_softmax.cpu().numpy()))
    ...
    def forward(self, inputs, A):
        if inputs.min() < 0 or inputs.max() >= self.embedding.num_embeddings:
            raise ValueError(f"[Invalid index] inputs contain out-of-range values: min={inputs.min().item()}, max={inputs.max().item()}, num_embeddings={self.embedding.num_embeddings}")
...
def train_test_ht_sl(model, train_data, test_data, group_dict, seed):
    ...
    global evaluation_log, score_log, recommended_items, novelty_scores
    evaluation_log = []
    score_log = []
    ...
    for i in slices:
        ...
        phic = 0
        input_sessions = test_data.inputs[i]
        session_ids = test_data.session_ids[i]
        for score, target, session, session_id in zip(sub_scores, targets, input_sessions, session_ids):
            ...
            recommended_item = [sc + 1 for sc in score.tolist()]

            score_log.append(recommended_item)
...
def train_test_ht_sl_loss(model, train_data, test_data, head_list, tail_list, seed):
    utils.set_random_seed(seed)
    ...
    item_counts = Counter()
    for session in train_data.inputs:
        item_counts.update(session)

    total_item_occurrences = sum(item_counts.values())
    item_prob = {item: count / total_item_occurrences for item, count in item_counts.items()}
    all_train_items = set(item_counts.keys())
    ...
    for i in slices:
        ...
        sub_scores = trans_to_cpu(sub_scores).detach().numpy()
        phic = 0
        for score, target, mask, length in zip(sub_scores, targets, test_data.mask):
            true_idx = target - 1
            h = np.isin(true_idx, score)
            m = 1 / (np.where(score == true_idx)[0][0] + 1) if true_idx in score else 0
            ...
            # 로그 저장
            evaluation_log.append((session, target, h, m, item_type))
            ...
            # coverage
            recommended_items.update(recommended_item)

            # novelty
            novelty = [-math.log2(item_prob.get(i, 1e-8)) for i in score]
    ...
    result = {
        ...
        'coverage': len(recommended_items) / len(all_train_items) * 100,
        'novelty': np.mean(novelty_scores),
    }

    return result, evaluation_log, score_log, total_loss
