...
class RecSysDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ### weight를 hook하기 위해 session_ids, is_original_flag 필요
        #session_ids = np.asarray(self.data[2][index]) if len(self.data) >= 3 else None
        session_ids = self.data[2][index] if len(self.data) >= 3 else None
        is_original_flags = np.asarray(self.data[3][index]) if len(self.data) >= 4 else None

        return session_items, target_item, session_ids, is_original_flags
